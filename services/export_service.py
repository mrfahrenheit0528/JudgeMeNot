    ...
    def generate_pdf(self, filepath, event_name, title, data_matrix, mode="segment"):
        ...
        def build_gender_section(gender_name, rows):
            ...
            header_style = ParagraphStyle(
                'HeaderStyle',
                parent=styles['Normal'],   # Inherit basic settings
                fontName='Helvetica-Bold', # Bold font
                fontSize=9,                # Size 9
                textColor=colors.white,    # White text (to match blue bg)
                alignment=TA_CENTER,       # Center aligned
                leading=11,                # Line spacing
                wordWrap='CJK'             # improved wrapping behavior
            )
            
            # 2. Table Data
            # We wrap every header string in a Paragraph object
            raw_headers = ["Rank", "#", "Candidate"] + cols + ["Total"]
            ...
            t.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.Color(0.2, 0.4, 0.6)), # Blue Header Background
                # REMOVED: ('TEXTCOLOR', ...) - The Paragraph style now handles the text color
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
                # REMOVED: ('FONTNAME', ...) - Paragraph handles font
                ('FONTSIZE', (0, 1), (-1, -1), 9), # Keep font size for DATA rows only
                ('BOTTOMPADDING', (0, 0), (-1, 0), 8),
                ('TOPPADDING', (0, 0), (-1, 0), 8), # Add padding for multi-line headers
